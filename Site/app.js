...
class DrawLineBehaviour extends DrawBehaviour {
    constructor(depth, color, width = 1) {
        super(color, depth);
        this.width = width;
    }
    draw(graphicsInterface, transform) {
...
class DrawTextBehaviour extends DrawBehaviour {
    constructor(text, depth, color, align = undefined, baseline = undefined, font = undefined) {
        super(color, depth);
        this.text = text;
        this.align = align;
...
class CounterGameObject extends GameEngine_1.GameObject {
    constructor() {
        ...
        this.ready = false;
        this.text = new GameEngine_1.DrawTextBehaviour("", 1000, 'white');
        ...
        back.drawBehaviour = new GameEngine_1.DrawRectBehaviour(999, 'rgba(0,0,0,0.3)');
...
class SensorGameObject extends GameEngine_1.GameObject {
    constructor(rotation, radius) {
        ...
        this.drawChild.drawBehaviour = new GameEngine_1.DrawLineBehaviour(-50, 'hsl(104, 15%, 50%)', 1);
...
class CarGameObject extends GameEngine_1.GameObject {
    constructor(behaviour, inputs, userCar, sensorLength) {
        ...
        this.bodyChild.drawBehaviour = new GameEngine_1.DrawRectBehaviour(depth, color);
        ...
        this.scoreTextBox = new GameEngine_1.DrawTextBehaviour('', depth + 2, borderColor);
        ...
        for (let rotation of [0, Math.PI * 0.5, Math.PI, Math.PI * 1.5]) {
            ...
            collider.drawBehaviour = new GameEngine_1.DrawLineBehaviour(depth + 1, borderColor);
            this.bodyChild.add(collider);
...
class WallGameObject extends PointPointGameObject {
    constructor(a, b) {
        ...
        this.drawBehaviour = new GameEngine_1.DrawLineBehaviour(10, 'rgb(85, 112, 75)', 2);
    }
}
class GroundGameObject extends GameEngine_1.GameObject {
    constructor(x, y, rotation, width, height) {
        super();
        this.drawBehaviour = new GameEngine_1.DrawRectBehaviour(-300, 'rgb(165, 192, 155)');
...
class Race {
    ...
    static generateMap(holder, params) {
        ...
        for (let i = 0; i < tracks.length; i++) {
            ...
            if (track.from == track.to) {
                ...
                if (i == 0) {
                    ...
                    lineGO.drawBehaviour = new GameEngine_1.DrawRectBehaviour(-290, 'hsl(104, 15%, 60%)');
                    go.add(lineGO);
                }
        ...
        background.drawBehaviour = new GameEngine_1.DrawRectBehaviour(-1000, 'hsl(104, 15%, 50%)');
...
$(() => {
	...
	"trackCornerCut": 0.5
        ...
        stopCondition: new Setting('Stop condition of race', '', `// (car: {lap: number, ai: boolean, alive: boolean}[], gameTime: number) => boolean
function(cars, gameTime) {`, `return cars.filter(c => c.ai && c.alive && c.lap < 2).length === 0 || gameTime > 20000;`, `}`, 'javascript', 'race-settings'),
        // NN settings
